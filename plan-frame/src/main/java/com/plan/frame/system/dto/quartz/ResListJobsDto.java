package com.plan.frame.system.dto.quartz;

import com.plan.frame.entity.PageinationDto;
import com.plan.frame.system.base.entity.SysQuartzJob;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;

import java.util.List;

/**
 * @Created by shenyiyi
 * @Description
 * @ClassName
 * @Create in 2020/4/14 18:35
 */
@ApiModel(description = "任务查询-应答对象")
public class ResListJobsDto extends PageinationDto {
    @ApiModelProperty("任务列表")
    private List<SysQuartzJob> jobList;

    public List<SysQuartzJob> getJobList() {
        return jobList;
    }

    public void setJobList(List<SysQuartzJob> jobList) {
        this.jobList = jobList;
    }
}
